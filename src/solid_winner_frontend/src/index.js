...
import { solid_winner_backend } from "../../declarations/solid_winner_backend";
...
document.addEventListener(
	...
	async (e) => {
		...
		const question = await solid_winner_backend.getQuestion();
		document.getElementById("question").innerText = question;

		const voteCounts = await solid_winner_backend.getVotes();
...
pollForm.addEventListener(
	...
	async (e) => {
		...
		const updatedVoteCounts = await solid_winner_backend.vote(checkedValue);
...
resetButton.addEventListener(
	...
	async (e) => {
		...
		await solid_winner_backend.resetVotes();
		const voteCounts = await solid_winner_backend.getVotes();
		updateLocalVoteCounts(voteCounts);
